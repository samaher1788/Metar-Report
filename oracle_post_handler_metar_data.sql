-- ✅ POST Handler النهائي لجدول METAR_DATA
-- ============================================
-- 
-- URI: /metar/save/
-- Method: POST
-- 
-- هذا الكود مُخصص 100% لجدول METAR_DATA
-- يحفظ سجل METAR واحد من نظام تقارير العواصف الغبارية

DECLARE
    v_body_text VARCHAR2(32767);
    v_metar_id NUMBER;
    
    -- بيانات المحطة والوقت
    v_station VARCHAR2(10);
    v_valid VARCHAR2(50);
    v_year NUMBER(4);
    v_month NUMBER(2);
    v_day NUMBER(2);
    v_hour NUMBER;
    v_minute NUMBER(2);
    
    -- بيانات الطقس
    v_temp_f NUMBER;
    v_temp_c NUMBER(4,1);
    v_dwpt_f NUMBER;
    v_dwpt_c NUMBER(4,1);
    v_wind_dir NUMBER(3);
    v_wind_speed NUMBER(4);
    v_visibility NUMBER(10,1);
    v_pressure NUMBER(5,1);
    v_wxcodes VARCHAR2(20);
    v_metar_msg VARCHAR2(200);
    
    -- متغيرات مساعدة
    v_timestamp TIMESTAMP;
    
BEGIN
    -- Parse JSON من body
    v_body_text := utl_raw.cast_to_varchar2(:body);
    apex_json.parse(v_body_text);
    
    -- استخراج القيم الأساسية
    v_station := apex_json.get_varchar2('station');
    v_valid := apex_json.get_varchar2('valid');
    v_metar_msg := apex_json.get_varchar2('metar');
    
    -- تحويل الوقت إلى timestamp ثم استخراج المكونات
    BEGIN
        v_timestamp := TO_TIMESTAMP(v_valid, 'YYYY-MM-DD HH24:MI');
        v_year := EXTRACT(YEAR FROM v_timestamp);
        v_month := EXTRACT(MONTH FROM v_timestamp);
        v_day := EXTRACT(DAY FROM v_timestamp);
        v_hour := EXTRACT(HOUR FROM v_timestamp);
        v_minute := EXTRACT(MINUTE FROM v_timestamp);
    EXCEPTION
        WHEN OTHERS THEN
            -- إذا فشل التحويل، استخدم القيم الافتراضية
            v_year := EXTRACT(YEAR FROM SYSDATE);
            v_month := EXTRACT(MONTH FROM SYSDATE);
            v_day := EXTRACT(DAY FROM SYSDATE);
            v_hour := 0;
            v_minute := 0;
    END;
    
    -- استخراج بيانات الطقس (بصيغة IEM - فهرنهايت وميل)
    v_temp_f := apex_json.get_number('tmpf');
    v_dwpt_f := apex_json.get_number('dwpf');
    v_wind_dir := apex_json.get_number('drct');
    v_wind_speed := apex_json.get_number('sknt');
    v_visibility := apex_json.get_number('vsby');
    v_pressure := apex_json.get_number('alti');
    v_wxcodes := apex_json.get_varchar2('wxcodes');
    
    -- تحويل درجة الحرارة من فهرنهايت إلى مئوي
    IF v_temp_f IS NOT NULL THEN
        v_temp_c := ROUND((v_temp_f - 32) * 5 / 9, 1);
    END IF;
    
    IF v_dwpt_f IS NOT NULL THEN
        v_dwpt_c := ROUND((v_dwpt_f - 32) * 5 / 9, 1);
    END IF;
    
    -- تحويل الرؤية من ميل إلى كيلومتر
    IF v_visibility IS NOT NULL THEN
        v_visibility := ROUND(v_visibility * 1.60934, 1);
    END IF;
    
    -- تحويل الضغط من inHg إلى hPa
    IF v_pressure IS NOT NULL THEN
        v_pressure := ROUND(v_pressure * 33.8639, 1);
    END IF;
    
    -- إدخال البيانات في الجدول
    INSERT INTO METAR_DATA (
        METAR_ID,
        STATION_ID,
        OBS_YEAR,
        OBS_MONTH,
        OBS_DAY,
        OBS_HOUR,
        OBS_MINUTE,
        TEMPERATURE_C,
        DEWPOINT_C,
        WIND_DIRECTION_DEG,
        WIND_SPEED_KT,
        VISIBILITY_KM,
        PHENOMENA_CODE,
        PRESSURE_QNH,
        MESSAGE_TYPE,
        METAR_MESSAGE,
        CREATED_BY,
        CREATED_ON
    ) VALUES (
        METAR_ID_SEQ.NEXTVAL,  -- تأكد من وجود Sequence بهذا الاسم
        v_station,
        v_year,
        v_month,
        v_day,
        v_hour,
        v_minute,
        v_temp_c,
        v_dwpt_c,
        v_wind_dir,
        v_wind_speed,
        v_visibility,
        v_wxcodes,
        v_pressure,
        'METAR',
        v_metar_msg,
        'AUTO_SYNC',
        SYSDATE
    ) RETURNING METAR_ID INTO v_metar_id;
    
    COMMIT;
    
    -- رد نجاح
    :status := 201;
    apex_json.open_object;
    apex_json.write('success', TRUE);
    apex_json.write('id', v_metar_id);
    apex_json.write('station', v_station);
    apex_json.write('message', 'تم حفظ البيانات بنجاح');
    apex_json.close_object;
    
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        -- في حالة تكرار البيانات
        ROLLBACK;
        :status := 200;
        apex_json.open_object;
        apex_json.write('success', TRUE);
        apex_json.write('duplicate', TRUE);
        apex_json.write('message', 'البيانات موجودة مسبقاً');
        apex_json.close_object;
        
    WHEN OTHERS THEN
        ROLLBACK;
        :status := 500;
        apex_json.open_object;
        apex_json.write('success', FALSE);
        apex_json.write('error', SQLERRM);
        apex_json.write('station', v_station);
        apex_json.close_object;
END;
